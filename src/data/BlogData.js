export const blogData = [
  {
    title: "Carnation (康乃馨 Kāng nǎi xīn)",
    img: "/images/blog-11.jpeg",
    desc: "Carnations symbolize love, admiration, and pure affection. Their ruffled petals and delicate fragrance make them one of the most timeless flowers used in elegant bouquets and celebrations.",
    note: "They are ideal for bouquets, gift arrangements, weddings, and special occasions, offering elegance, freshness, and emotional expression in every bloom.",
    maintain: [
      "Carnations are timeless blooms that speak the language of love and admiration. Their soft, ruffled petals create a graceful and charming appearance, while each flower carries a sense of purity, warmth, and heartfelt emotion. Loved across cultures, carnations symbolize affection that lasts forever, making them a classic choice for meaningful moments and elegant gifting.",
    ],

    origin:
      "Known for their gentle fragrance, carnations add a soothing presence to bouquets, while their layered petals bring volume, texture, and refined beauty. Available in delicate pastels and vibrant shades, they suit every celebration. From joyful occasions to heartfelt expressions, carnations never feel out of place, effortlessly balancing tradition with modern floral styling.",

    symbolic: [
      "Carnations are cherished for both their beauty and long-lasting freshness. They represent devotion, gratitude, and pure emotional connections, making them perfect for bouquets, arrangements, and decorative floral designs. Their elegant form enhances weddings, events, and everyday surprises, remaining a flower that is graceful, meaningful, and unforgettable.",
    ],
    careContent:
      "To keep your carnations fresh and vibrant, ensure they receive plenty of water. Change the water every two days and trim the stems at an angle. Keep them away from direct sunlight and ripening fruits, as these can shorten their vase life.",

    CareTips: [
      {
        Tip: "Trim stems at an angle before placing in fresh water",
      },
      {
        Tip: "Use a sturdy, clean vase with plenty of water",
      },
      {
        Tip: "Change water every 1–2 days to maintain freshness",
      },
      {
        Tip: "Keep in a cool area, away from direct sunlight and heat",
      },
      {
        Tip: "Remove leaves below the waterline to prevent bacteria",
      },
    ],
    Culture:
      "Carnations are cherished for both their beauty and long-lasting freshness. They represent devotion,gratitude, and pure emotional connections, making them perfect for bouquets, arrangements, and decorativefloral designs. Their elegant form enhances weddings, events, and everyday surprises, remaining a flower that is graceful, meaningful, and unforgettable.",
  },
  {
    title: "Gerbera Daisies (非洲菊 Fēizhōu jú)",
    img: "/images/blog-13.jpg",
    note: "Gerbera Daisies are cherished for their vivid colors, cheerful appearance, and long-lasting freshness. Ideal for expressing happiness, admiration, and heartfelt emotions, they make every bouquet and floral arrangement lively and memorable.",
    desc: "Gerberas are cheerful, bold blossoms known for their bright colors and joyful charm. They instantly add positivity and freshness to any bouquet with their large daisy-like petals.",
    maintain: [
      "Gerbera Daisies are vibrant blooms that radiate joy and cheerfulness, with bold, colorful petals that instantly brighten any bouquet or arrangement. Symbolizing purity, innocence, and happiness, they bring smiles wherever they go. Each flower carries a lively spirit, making them perfect for celebrating life’s precious moments, and they remain a favorite choice for expressing admiration and heartfelt emotions.",
    ],

    origin:
      "With their striking colors and elegant form, gerberas create a visual delight, ranging from soft pastels to radiant hues that complement any floral design beautifully. These flowers effortlessly combine playfulness with sophisticated charm, making them ideal for birthdays, anniversaries, or simply to lift someone’s spirits. Gerberas bring energy, positivity, and a touch of elegance to every bouquet.",

    symbolic: [
      "Gerbera Daisies are loved for their long-lasting freshness and versatility, symbolizing cheer, gratitude, and genuine affection in every petal. Perfect for bouquets, centerpieces, and decorative arrangements alike, their vibrant presence enhances celebrations, events, and everyday moments. A flower that spreads happiness, warmth, and unforgettable memories, gerberas remain a delightful choice for any occasion.",
    ],
    careContent:
      "To keep your gerbera daisies bright and cheerful, provide them with fresh water and change it every two days. Trim the stems at an angle to improve water absorption. Place them in a cool spot, away from direct sunlight, heat, and ripening fruits to ensure they stay vibrant for longer.",

    CareTips: [
      {
        Tip: "Trim stems at an angle every two days.",
      },
      {
        Tip: "Change water regularly to prevent bacterial growth.",
      },
      {
        Tip: "Keep away from direct sunlight and heat sources.",
      },
      {
        Tip: "Avoid placing near ripening fruits, which release ethylene gas.",
      },
    ],
    Culture:
      "Gerbera daisies are admired for their bright, cheerful colors and playful charm. They symbolize happiness, innocence, and purity of heart, making them perfect for bouquets, floral arrangements, and decorative displays. Their vibrant presence brings joy to weddings, celebrations, and everyday moments, remaining a flower that is lively, meaningful, and unforgettable.",
  },
  {
    title: "Hydrangea (绣球花 Xiù qiú huā)",
    img: "/images/blog-12.jpg",
    note: "Hydrangeas are admired for their lush blooms, soft fragrance, and elegant appearance. Symbolizing gratitude and heartfelt emotions, they are ideal for adding volume, texture, and sophistication to bouquets, arrangements, and special occasions.",
    desc: "Hydrangeas are lush, full flowers that symbolize heartfelt emotions.Their soft, cloud-like clusters create a luxurious and romantic touch in premium bouquets.",
    maintain: [
      "Hydrangeas are enchanting blooms that embody grace and abundance, with lush, rounded clusters that create a sense of fullness and elegance. Symbolizing heartfelt emotions, gratitude, and deep understanding, they speak volumes without words. Each bloom radiates softness and sophistication, making them perfect for refined arrangements and cherished for making celebrations and special moments truly memorable.",
    ],

    origin:
      "With their pastel shades and vibrant tones, hydrangeas complement any floral design, effortlessly adding texture, volume, and a touch of luxury to bouquets. Ideal for weddings, anniversaries, or heartfelt gifts, these flowers exude charm, while their delicate petals carry a gentle fragrance that soothes and captivates. Hydrangeas blend timeless beauty with modern elegance, making them truly irresistible.",

    symbolic: [
      "Hydrangeas are loved for their versatility and long-lasting freshness, symbolizing gratitude, understanding, and heartfelt emotion in every cluster. Perfect for bouquets, centerpieces, or decorative arrangements alike, their presence enhances events, celebrations, and even everyday spaces with grace. A flower that embodies sophistication, emotion, and unforgettable beauty, hydrangeas remain a timeless and elegant choice for any occasion.",
    ],
    careContent:
      "To keep your hydrangeas lush and vibrant, make sure they receive plenty of water, as they are highly thirsty flowers. Change the water every two days and trim the stems at an angle to improve water uptake. Keep them in a cool area, away from direct sunlight, heat, and ripening fruits to maintain their full, beautiful blooms.",

    CareTips: [
      {
        Tip: "Trim stems at an angle every two days.",
      },
      {
        Tip: "Change water regularly to prevent bacterial growth.",
      },
      {
        Tip: "Keep away from direct sunlight and heat sources.",
      },
      {
        Tip: "Avoid placing near ripening fruits, which release ethylene gas.",
      },
    ],
    Culture:
      "Hydrangeas are admired for their lush, full blooms and delicate beauty. They symbolize gratitude, understanding, and heartfelt emotions, making them perfect for bouquets, arrangements, and decorative displays. Their elegant presence enhances weddings, special occasions, and everyday moments, remaining a flower that is graceful, meaningful, and unforgettable.",
  },
  {
    title: "Lily (百合花 Bǎi hé huā)",
    img: "/images/blog-17.jpeg",
    note: "Lilies are cherished for their elegance, delicate fragrance, and symbolic meaning of purity and devotion. Ideal for bouquets, arrangements, and special occasions, they bring sophistication, emotional depth, and timeless beauty to every floral display.",
    desc: "Lilies are elegant, fragrant flowers that represent purity and renewal.Their graceful petals bring a touch of sophistication and beauty to every arrangement.",
    maintain: [
      "Lilies are graceful blooms that symbolize purity, devotion, and refined beauty, with trumpet-shaped petals that create a striking and elegant appearance. Each flower carries a sense of peace, renewal, and heartfelt emotion, and loved across cultures, lilies are often used to convey respect and admiration. They are a timeless choice for celebrations, gifts, and sophisticated floral arrangements.",
    ],

    origin:
      "With their vibrant colors and delicate fragrance, lilies bring charm and freshness to any bouquet, adding height, structure, and a touch of elegance to floral designs. From weddings and anniversaries to heartfelt expressions of love, lilies suit every occasion. Their serene beauty evokes calm, grace, and emotional warmth in every setting, effortlessly balancing classic symbolism with modern floral aesthetics.",

    symbolic: [
      "Lilies are admired for their long-lasting freshness and versatile appeal, representing purity, devotion, and heartfelt emotion in every petal. Perfect for bouquets, centerpieces, or decorative arrangements alike, their presence enhances events, celebrations, and everyday moments with elegance. A flower that embodies grace, emotion, and timeless beauty, lilies leave a lasting impression.",
    ],
    careContent:
      "To keep your lilies fresh and fragrant, provide them with plenty of water and change it every two days. Trim the stems at an angle to help them absorb water efficiently. Keep lilies in a cool area, away from direct sunlight, heat sources, and ripening fruits to preserve their blooms and fragrance.",

    CareTips: [
      {
        Tip: "Trim stems at an angle every two days.",
      },
      {
        Tip: "Change water regularly to prevent bacterial growth.",
      },
      {
        Tip: "Keep away from direct sunlight and heat sources.",
      },
      {
        Tip: "Avoid placing near ripening fruits, which release ethylene gas.",
      },
    ],
    Culture:
      "Lilies are celebrated for their exquisite beauty and graceful form. They symbolize purity, renewal, and refined elegance, making them ideal for bouquets, floral arrangements, and decorative displays. Their timeless charm enhances weddings, special occasions, and everyday moments, remaining a flower that is sophisticated, meaningful, and unforgettable.",
  },
  {
    title: "Phalaenopsis Orchids (蝴蝶兰 Húdiélán)",
    img: "/images/blog-29.jpg",
    note: "Phalaenopsis Orchids are admired for their delicate beauty, long-lasting blooms, and graceful elegance. Symbolizing love, sophistication, and refinement, they are ideal for bouquets, arrangements, and special occasions, adding a luxurious and timeless touch to any floral display.",
    desc: "Known as “moth orchids,” these elegant blooms bring a sense of luxury and harmony. Their curved petals resemble fluttering butterflies, symbolizing beauty and long-lasting love.",
    maintain: [
      "Phalaenopsis Orchids are exquisite blooms that symbolize elegance, beauty, and sophistication, with delicate, butterfly-like petals that create a mesmerizing and graceful appearance. Each flower embodies refinement, charm, and a sense of serenity. Loved for their long-lasting blooms, these orchids are perfect for special celebrations and are a distinguished choice for gifting, decorating, and elevating any floral arrangement.",
    ],

    origin:
      "With their soft, vibrant colors and luxurious form, Phalaenopsis Orchids add a touch of sophistication to any bouquet, bringing texture, balance, and an air of refinement to floral designs. Ideal for weddings, anniversaries, corporate gifting, or heartfelt expressions, these orchids suit every occasion. Their serene elegance and understated beauty captivate and soothe the soul, effortlessly combining timeless grace with modern floral aesthetics.",

    symbolic: [
      "Phalaenopsis Orchids are cherished for their versatility and long-lasting charm, symbolizing love, admiration, and delicate beauty in every bloom. Perfect for bouquets, centerpieces, or ornamental arrangements alike, their presence elevates events, celebrations, and even everyday spaces with poise. A flower that embodies sophistication, elegance, and unforgettable allure, Phalaenopsis Orchids leave a lasting impression.",
    ],
    careContent:
      "To keep your Phalaenopsis orchids healthy and blooming, water them moderately, allowing the potting medium to dry slightly between waterings. Avoid overwatering, as this can cause root rot. Place your orchids in bright, indirect sunlight and maintain a humid environment for optimal growth. Keep them away from drafts and ripening fruits to protect their delicate flowers.",

    CareTips: [
      {
        Tip: "Water moderately and let the medium dry slightly between waterings.",
      },
      {
        Tip: "Avoid overwatering to prevent root rot.",
      },
      {
        Tip: "Place in bright, indirect sunlight.",
      },
      {
        Tip: "Maintain humidity around the plant for healthy blooms.",
      },
      {
        Tip: "Keep away from drafts and ripening fruits that release ethylene gas.",
      },
    ],
    Culture:
      "Phalaenopsis orchids, are admired for their elegant, butterfly-like blooms and delicate sophistication. They symbolize beauty, refinement, and thoughtfulness, making them perfect for bouquets, decorative arrangements, and special gifts. Their graceful presence elevates weddings, celebrations, and everyday moments, remaining a flower that is luxurious, meaningful, and unforgettable.",
  },
  {
    title: "Peonies (蝴蝶兰 Húdiélán)",
    img: "/images/blog-22.jpg",
    note: "Peonies are cherished for their lush, layered blooms, delicate fragrance, and symbolic meanings of love and prosperity. Perfect for bouquets, arrangements, and special occasions, they bring elegance, charm, and timeless beauty to any floral display.",
    desc: "Peonies are lush, romantic blooms known for their full, soft petals and elegant fragrance. They symbolize prosperity and deep love, making bouquets appear rich, dreamy, and premium.",
    maintain: [
      "Peonies are lush, captivating blooms that symbolize prosperity, romance, and elegance, with full, layered petals that create a sense of abundance and grace. Each flower radiates charm, beauty, and heartfelt emotion, and celebrated across cultures, peonies represent good fortune and love. They are a timeless choice for weddings, gifts, and sophisticated floral arrangements.",
    ],

    origin:
      "With their rich colors and delicate fragrance, peonies enhance any bouquet or centerpiece, adding texture, volume, and a luxurious touch to floral designs. Perfect for anniversaries, special occasions, or heartfelt expressions, peonies suit every moment. Their elegance and vibrant presence captivate and inspire admiration, effortlessly blending traditional symbolism with modern floral artistry.",

    symbolic: [
      "Peonies are admired for their long-lasting freshness and versatile appeal, symbolizing romance, prosperity, and heartfelt emotion in every petal. Ideal for bouquets, centerpieces, or decorative arrangements alike, their presence elevates celebrations, events, and everyday spaces with beauty. A flower that embodies elegance, emotion, and timeless allure, peonies leave an unforgettable impression.",
    ],
    careContent:
      "To keep your peonies fresh and vibrant, provide them with plenty of water and change it every two days Trim the stems at an angle to help them absorb water better. Keep them in a cool spot, away from direct sunlight, heat sources, and ripening fruits to maintain their lush, full blooms.",

    CareTips: [
      {
        Tip: "Trim stems at an angle every two days.",
      },
      {
        Tip: "Change water regularly to prevent bacterial growth.",
      },
      {
        Tip: "Keep away from direct sunlight and heat sources.",
      },
      {
        Tip: "Avoid placing near ripening fruits, which release ethylene gas.",
      },
    ],
    Culture:
      "Peonies are cherished for their lush, full blooms and captivating fragrance. They symbolize prosperity, romance, and honor, making them perfect for bouquets, decorative arrangements, and festive occasions. Their majestic presence enhances weddings, celebrations, and everyday moments, remaining a flower that is elegant, meaningful, and unforgettable.",
  },
  {
    title: "Ranunculus (花毛茛 Huā máogèn)",
    img: "/images/blog-21.jpg",
    note: "Ranunculus are cherished for their vibrant colors, delicate layered petals, and long-lasting freshness. Symbolizing charm, joy, and heartfelt emotion, they are ideal for bouquets, arrangements, and special occasions, adding elegance, texture, and a cheerful touch to any floral display.",
    desc: "Ranunculus flowers look like soft, layered paper petals. Their delicate, artistic appearance adds a refined and stylish look to bouquets, often used for high-end arrangements.",
    maintain: [
      "Ranunculus are charming blooms known for their layered, delicate petals and vibrant colors. Each flower radiates joy, elegance, and playful sophistication, making it a perfect choice for special occasions and stylish bouquets. Celebrated for their bright and cheerful appearance, ranunculus flowers bring warmth, beauty, and a sense of delight to any floral arrangement. Their refined form and intricate structure make them a favorite among florists and flower enthusiasts alike.",
    ],

    origin:
      "Available in a wide range of pastel shades and bold hues, ranunculus flowers add depth, texture, and vibrancy to bouquets and centerpieces. They complement both modern and classic floral designs, effortlessly enhancing the aesthetic appeal of weddings, celebrations, or heartfelt gifts. With their soft petals and charming presence, ranunculus blooms convey admiration, joy, and affection, leaving a lasting impression on anyone who receives them.",

    symbolic: [
      "Ranunculus are admired not only for their stunning beauty but also for their long-lasting freshness and versatility. Perfect for bouquets, decorative arrangements, or as standalone blooms, they bring elegance, color, and charm to every setting. These flowers symbolize radiant charm, attractiveness, and heartfelt emotions, making them a timeless choice for meaningful gifts or luxurious floral displays. Their presence enhances events, celebrations, and everyday moments with grace and sophistication.",
    ],
    careContent:
      "To keep your ranunculus flowers bright and full, provide them with fresh water and change it every two days. Trim the stems at an angle to help them absorb water effectively. Keep them in a cool area, away from direct sunlight, heat, and ripening fruits, to maintain their vibrant, layered blooms.",

    CareTips: [
      {
        Tip: "Trim stems at an angle every two days.",
      },
      {
        Tip: "Change water regularly to prevent bacterial growth.",
      },
      {
        Tip: "Keep away from direct sunlight and heat sources.",
      },
      {
        Tip: "Avoid placing near ripening fruits, which release ethylene gas.",
      },
    ],
    Culture:
      "Ranunculus flowers are admired for their layered, delicate petals and vibrant colors. They symbolize charm, attractiveness, and radiant beauty, making them ideal for bouquets, decorative arrangements, and special gifts. Their graceful presence brightens weddings, celebrations, and everyday moments, remaining a flower that is lively, meaningful, and unforgettable.",
  },
  {
    title: "Roses (玫瑰 Méiguī)",
    img: "/images/blog-16.jpeg",
    note: "Roses are cherished for their velvety petals, enchanting fragrance, and symbolic meanings of love, admiration, and purity. Perfect for bouquets, centerpieces, and special occasions, they bring elegance, emotion, and timeless beauty to every floral display.",
    desc: "Roses are timeless symbols of love and emotion. Their rich colours, velvety petals, and classic fragrance make them perfect for every celebration, from romance to friendship.",
    maintain: [
      "Roses are timeless and iconic blooms, celebrated for their beauty, elegance, and symbolic meanings of love and admiration. Their velvety petals and enchanting fragrance make them a quintessential choice for bouquets and floral arrangements. Each rose conveys deep emotions, from passion and romance to gratitude and respect, making them perfect for any heartfelt occasion.",
    ],

    origin:
      "Available in a stunning variety of colors, roses can express different sentiments and enhance any floral design. From classic red roses symbolizing love to soft pinks and whites for admiration and purity, their versatility is unmatched. Ideal for weddings, anniversaries, celebrations, or personal gestures, roses bring charm, sophistication, and emotion to every bouquet.",

    symbolic: [
      "Roses are admired for their long-lasting freshness and timeless appeal. They add depth, texture, and elegance to bouquets, centerpieces, and decorative arrangements alike. A flower that embodies beauty, love, and heartfelt emotion, roses leave a lasting impression and are cherished across cultures and generations.",
    ],
    careContent:
      "To keep your roses fresh, fragrant, and long-lasting, provide them with plenty of clean water and change it every two days. Trim the stems at an angle to improve water absorption. Keep them in a cool spot, away from direct sunlight, heat sources, and ripening fruits to preserve their blooms and vibrant color.",

    CareTips: [
      {
        Tip: "Trim stems at an angle every two days.",
      },
      {
        Tip: "Change water regularly to prevent bacterial growth.",
      },
      {
        Tip: "Keep away from direct sunlight and heat sources.",
      },
      {
        Tip: "Avoid placing near ripening fruits, which release ethylene gas.",
      },
    ],
    Culture:
      "Roses are celebrated for their timeless beauty and captivating fragrance. They symbolize love, passion, and admiration, making them perfect for bouquets, decorative arrangements, and meaningful gifts. Their elegant presence enhances weddings, celebrations, and everyday moments, remaining a flower that is classic, meaningful, and unforgettable.",
  },
  {
    title: "Sunflower (向日葵 Xiàng rì kuí)",
    img: "/images/blog-15.jpeg",
    note: "Sunflowers are cherished for their radiant petals, long-lasting freshness, and symbolic meanings of happiness and positivity. Ideal for bouquets, arrangements, and joyful occasions, they bring warmth, cheer, and vibrant energy to any floral display.",
    desc: "Sunflowers are bright, cheerful flowers that symbolize positivity and renewal. Their bold yellow petals stand for warmth, strength, and happiness, making them a crowd-favourite.",
    maintain: [
      "Sunflowers are radiant blooms that symbolize happiness, warmth, and positivity. Their large, golden petals and bold appearance instantly brighten any bouquet or floral arrangement. Each flower exudes cheerfulness and vitality, making it a perfect choice for celebrating joyful moments and spreading smiles. Loved across cultures, sunflowers are a universal symbol of optimism and light.",
    ],

    origin:
      "With their striking colors and tall, elegant stems, sunflowers add energy, texture, and a vibrant touch to floral designs. They complement both rustic and modern arrangements, creating a stunning visual impact. Ideal for birthdays, celebrations, and heartfelt gifts, sunflowers bring warmth and cheer wherever they are displayed. Their sunny presence lifts spirits and radiates positivity.",

    symbolic: [
      "Sunflowers are admired for their long-lasting freshness and versatile appeal. They represent happiness, loyalty, and adoration in every bloom. Perfect for bouquets, centerpieces, or decorative arrangements, they enhance celebrations, events, and everyday spaces with joy and elegance. A flower that embodies brightness, cheer, and unforgettable charm, sunflowers leave a lasting impression.",
    ],
    careContent:
      "To keep your sunflowers bright and cheerful, provide them with plenty of water and change it every two days. Trim the stems at an angle to help them absorb water efficiently. Place them in a cool area, away from direct sunlight, heat sources, and ripening fruits to ensure their vibrant blooms last longer.",

    CareTips: [
      {
        Tip: "Trim stems at an angle every two days.",
      },
      {
        Tip: "Change water regularly to prevent bacterial growth",
      },
      {
        Tip: "Keep away from direct heat sources.",
      },
      {
        Tip: "Avoid placing near ripening fruits, which release ethylene gas.",
      },
    ],
    Culture:
      "Sunflowers are admired for their bright, sunny appearance and uplifting charm. They symbolize happiness, loyalty, and positivity, making them perfect for bouquets, decorative arrangements, and joyful gifts. Their radiant presence enhances celebrations, special occasions, and everyday moments, remaining a flower that is cheerful, meaningful, and unforgettable.",
  },
  {
    title: "Tulip (郁金香 Yùjīnxiāng)",
    img: "/images/blog-14.jpeg",
    note: "Tulips are cherished for their smooth, elegant petals, long-lasting freshness, and symbolic meanings of love, cheerfulness, and grace. Ideal for bouquets, arrangements, and special occasions, they bring vibrancy, warmth, and timeless beauty to any floral display.",
    desc: "Tulips are elegant, modern flowers with smooth petals and clean lines. Their simplicity adds sophistication and a fresh look to bouquets, especially in pastel colours.",
    maintain: [
      "Tulips are graceful blooms that symbolize elegance, love, and cheerfulness. Their smooth, cup-shaped petals create a simple yet striking beauty, making them a favorite for bouquets and floral arrangements. Each flower carries warmth, joy, and heartfelt emotion, perfect for celebrating special moments or expressing sincere feelings. Tulips are cherished across cultures for their timeless charm and versatile appeal.",
    ],

    origin:
      "Available in a vibrant spectrum of colors, tulips can convey a wide range of emotions—from passionate red to gentle pink, sunny yellow, and pure white. Their sleek stems and elegant form add sophistication and balance to any bouquet or centerpiece. Ideal for weddings, anniversaries, or thoughtful gifts, tulips bring freshness, vibrancy, and a cheerful presence to every floral design.",

    symbolic: [
      "Tulips are admired for their long-lasting freshness and refined simplicity. They symbolize love, grace, and positivity in every bloom. Perfect for bouquets, decorative arrangements, or centerpieces, tulips enhance celebrations, events, and everyday spaces with elegance and warmth. A flower that embodies beauty, emotion, and timeless charm, tulips leave a memorable impression.",
    ],
    careContent:
      "To keep your tulips fresh and vibrant, provide them with plenty of water and change it every two days. Trim the stems at an angle to improve water absorption. Keep tulips in a cool area, away from direct sunlight, heat sources, and ripening fruits to maintain their elegant blooms.",

    CareTips: [
      {
        Tip: "Trim stems at an angle every two days.",
      },
      {
        Tip: "Change water regularly to prevent bacterial growth.",
      },
      {
        Tip: "Keep away from direct sunlight and heat sources.",
      },
      {
        Tip: "Avoid placing near ripening fruits, which release ethylene gas.",
      },
    ],
    Culture:
      "Tulips are celebrated for their vibrant colors and graceful form. They symbolize love, renewal, and cheerful emotions, making them perfect for bouquets, decorative arrangements, and thoughtful gifts. Their elegant presence enhances weddings, celebrations, and everyday moments, remaining a flower that is timeless, meaningful, and unforgettable.",
  },
  {
    title: "Alstroemeria (六出花 Liù chū huā)",
    img: "/images/blog-19.jpg",
    note: "Alstroemeria is treasured for its delicate, patterned petals, long-lasting freshness, and symbolic meanings of friendship, devotion, and prosperity. Ideal for bouquets, arrangements, and special occasions, it brings elegance, charm, and a cheerful presence to any floral display.",
    desc: "Alstroemeria, also known as the Peruvian Lily, features soft trumpetshaped petals with natural streaks. It adds elegance, colour, and fullness to bouquets, making arrangements look rich and lively.",
    maintain: [
      "Alstroemeria, also known as the Peruvian Lily, is a vibrant bloom that symbolizes friendship, devotion, and prosperity. Its delicate, trumpet-shaped petals are often patterned with striking streaks or spots, adding charm and visual interest to any bouquet. Each flower conveys warmth, care, and thoughtful sentiment, making it a perfect choice for expressing heartfelt emotions. Alstroemeria is cherished for its graceful appearance and timeless appeal.",
    ],

    origin:
      "With a variety of colors ranging from soft pastels to bold and vibrant hues, Alstroemeria adds depth, texture, and elegance to floral arrangements. Its slender stems and intricate petals make it ideal for bouquets, centerpieces, or decorative designs. Perfect for birthdays, anniversaries, celebrations, or as a gesture of appreciation, Alstroemeria flowers brighten spaces and uplift moods with their cheerful presence.",

    symbolic: [
      "Alstroemeria is admired for its long-lasting freshness and versatility. It symbolizes friendship, devotion, and heartfelt emotion in every bloom. Perfect for bouquets, decorative arrangements, or floral displays, it adds elegance, charm, and a touch of sophistication to any setting. A flower that embodies beauty, warmth, and enduring appeal, Alstroemeria leaves a memorable impression wherever it is displayed.",
    ],
    careContent:
      "To keep your Alstroemeria flowers fresh and vibrant, provide them with plenty of water and change it every two days. Trim the stems at an angle to help them absorb water efficiently. Keep them in a cool spot, away from direct sunlight, heat sources, and ripening fruits, to maintain their long-lasting, colorful blooms.",

    CareTips: [
      {
        Tip: "Trim stems at an angle every two days.",
      },
      {
        Tip: "Change water regularly to prevent bacterial growth.",
      },
      {
        Tip: "Keep away from direct sunlight and heat sources.",
      },
      {
        Tip: "Avoid placing near ripening fruits, which release ethylene gas.",
      },
    ],
    Culture:
      "Alstroemeria, also known as the Peruvian lily, is admired for its vibrant colors and intricate patterns. It symbolizes friendship, devotion, and mutual support, making it perfect for bouquets, decorative arrangements, and meaningful gifts. Its lively presence enhances celebrations, special occasions, and everyday moments, remaining a flower that is cheerful, meaningful, and unforgettable.",
  },
  {
    title: "Baby’s Breath (满天星 Mǎn tiān xīng)",
    img: "/images/blog-6.jpeg",
    note: "Baby’s Breath is treasured for its tiny, cloud-like blooms, long-lasting freshness, and symbolic meanings of purity, innocence, and love. Ideal as a filler or accent in bouquets and arrangements, it adds softness, elegance, and timeless charm to any floral display.",
    desc: "Baby’s Breath is known for its tiny cloud-like clusters of white blooms.It adds softness, volume, and purity to floral arrangements and is one of the most classic fillers worldwide.",
    maintain: [
      "Baby’s Breath is a delicate and airy bloom that symbolizes purity, innocence, and everlasting love. Its tiny, cloud-like white flowers create a soft, ethereal presence in any bouquet or arrangement. Often used as a filler, Baby’s Breath adds lightness and elegance, enhancing the beauty of surrounding blooms while standing out with its own subtle charm. It is cherished for its simplicity, grace, and timeless appeal.",
    ],

    origin:
      "With its gentle and versatile nature, Baby’s Breath complements a wide variety of flowers, from roses and lilies to peonies and tulips. Its delicate clusters create texture, depth, and a dreamy effect in bouquets and floral arrangements. Perfect for weddings, celebrations, or thoughtful gifts, Baby’s Breath adds a touch of romance, serenity, and sophistication to every design.",

    symbolic: [
      "Baby’s Breath is admired for its long-lasting freshness and effortless versatility. Symbolizing purity, love, and innocence, it is perfect for bouquets, centerpieces, and decorative arrangements alike. Its presence enhances events, celebrations, and even everyday spaces with delicate charm. A flower that embodies grace, elegance, and timeless beauty, Baby’s Breath leaves a gentle and unforgettable impression.",
    ],
    careContent:
      "To keep your Baby’s Breath fresh and airy, provide them with plenty of water and change it every two days. Trim the stems at an angle to improve water absorption. Keep them in a cool area, away from direct sunlight, heat sources, and ripening fruits to maintain their delicate, cloud-like blooms.",

    CareTips: [
      {
        Tip: "Trim stems at an angle every two days.",
      },
      {
        Tip: "Change water regularly to prevent bacterial growth.",
      },
      {
        Tip: "Keep away from direct sunlight and heat sources.",
      },
      {
        Tip: "Avoid placing near ripening fruits, which release ethylene gas.",
      },
    ],
    Culture:
      "Baby’s Breath is admired for its tiny, cloud-like blooms and delicate charm. It symbolizes purity, everlasting love, and innocence, making it perfect for bouquets, decorative arrangements, and meaningful accents in floral designs. Its gentle presence enhances weddings, celebrations, and everyday moments, remaining a flower that is soft, meaningful, and unforgettable.",
  },
  {
    title: "Caspia (霞草 Xiá cǎo)",
    img: "/images/blog-33.jpg",
    note: "Caspia is treasured for its delicate, wispy clusters, long-lasting freshness, and subtle elegance. Ideal as a filler or accent in bouquets and arrangements, it adds texture, refinement, and a dreamy, sophisticated charm to any floral display.",
    desc: "Caspia has feathery stems with tiny flower clusters in purple, white, or pink. It brings a wild, textured, and airy style to bouquets, making them look more dimensional and elegant.",
    maintain: [
      "Caspia is a delicate, airy bloom known for its fine, wispy stems and tiny, clustered flowers. Often used as a filler in bouquets and arrangements, Caspia adds texture, volume, and a soft, ethereal charm. Its subtle beauty complements more vibrant blooms while standing out with its own understated elegance. Cherished for its lightness and grace, Caspia enhances the sophistication of any floral design.",
    ],

    origin:
      "With its versatile form and soft hues, Caspia brings depth, balance, and refinement to bouquets and centerpieces. It pairs beautifully with roses, peonies, lilies, and other prominent flowers, adding a dreamy and elegant effect. Ideal for weddings, anniversaries, celebrations, or decorative arrangements, Caspia elevates every floral display with its airy and enchanting presence.",

    symbolic: [
      "Caspia is admired for its long-lasting freshness and adaptability in floral designs. Symbolizing elegance, tranquility, and subtle beauty, it is perfect for bouquets, centerpieces, or decorative accents. Its presence enhances events, celebrations, and even everyday spaces with delicate charm. A flower that embodies lightness, sophistication, and timeless appeal, Caspia leaves a memorable impression wherever it is used.",
    ],
    careContent:
      "To keep your Caspia flowers fresh and vibrant, provide them with plenty of water and change it every two days. Trim the stems at an angle to help them absorb water efficiently. Keep them in a cool area, away from direct sunlight, heat sources, and ripening fruits, to maintain their delicate, airy blooms.",

    CareTips: [
      {
        Tip: "Trim stems at an angle every two days.",
      },
      {
        Tip: "Change water regularly to prevent bacterial growth.",
      },
      {
        Tip: "Keep away from direct sunlight and heat sources.",
      },
      {
        Tip: "Avoid placing near ripening fruits, which release ethylene gas.",
      },
    ],
    Culture:
      "Caspia is admired for its delicate, airy clusters of tiny blooms and subtle elegance. It symbolizes remembrance, grace, and lasting beauty, making it perfect for bouquets, decorative arrangements, and thoughtful gifts. Its refined presence enhances weddings, celebrations, and everyday moments, remaining a flower that is gentle, meaningful, and unforgettable.",
  },
  {
    title: "Delphinium (飞燕草 Fēiyàn cǎo)",
    img: "/images/blog-10.jpg",
    note: "Delphinium is treasured for its tall, elegant spikes, vibrant colors, and symbolic meanings of dignity and admiration. Ideal for bouquets and arrangements, it adds height, structure, and a refined, luxurious touch to any floral display.",
    desc: "Delphiniums are tall, elegant spikes of clustered blooms that bring height, richness, and a soft romantic touch to bouquets. Their unique shape and vibrant blues, purples, and whites add sophistication to floral designs.",
    maintain: [
      "Delphinium is a striking, tall bloom known for its graceful spires and vibrant presence. Symbolizing dignity, grace, and bold beauty, it adds a sense of drama and elegance to any floral arrangement. Its clustered blossoms create a refined vertical structure, making delphinium a standout choice for sophisticated bouquets and statement designs.",
    ],

    origin:
      "Available in captivating shades of blue, purple, pink, and white, delphinium brings depth, texture, and height to floral compositions. It pairs beautifully with round and soft blooms such as roses, peonies, and lisianthus, enhancing both modern and classic styles. Ideal for weddings, celebrations, and special events, delphinium adds a majestic and uplifting charm to every setting.",

    symbolic: [
      "Delphinium is admired for its long-lasting freshness and expressive beauty. Representing admiration, positivity, and heartfelt emotion, it is perfect for bouquets, centerpieces, and decorative arrangements alike. Its presence elevates celebrations, events, and everyday spaces with elegance and confidence. A flower that embodies strength, grace, and timeless appeal, delphinium leaves a lasting impression.",
    ],
    careContent:
      "To keep your Delphinium flowers fresh and vibrant, provide them with plenty of water and change it every two days. Trim the stems at an angle to improve water absorption. Place them in a cool area, away from direct sunlight, heat sources, and ripening fruits, to maintain their tall, striking, and colorful blooms.",

    CareTips: [
      {
        Tip: "Trim stems at an angle every two days.",
      },
      {
        Tip: "Change water regularly to prevent bacterial growth.",
      },
      {
        Tip: "Keep away from direct sunlight and heat sources.",
      },
      {
        Tip: "Avoid placing near ripening fruits, which release ethylene gas.",
      },
    ],
    Culture:
      "Delphiniums are admired for their tall, striking spikes and vibrant colors. They symbolize positivity, dignity, and a sense of lightness, making them perfect for bouquets, decorative arrangements, and special gifts. Their elegant presence enhances weddings, celebrations, and everyday moments, remaining a flower that is uplifting, meaningful, and unforgettable.",
  },
  {
    title: "Freesia (小苍兰 Xiǎo cānglán)",
    img: "/images/blog-9.jpg",
    note: "Freesia is loved for its sweet fragrance, delicate petals, and symbolic meanings of friendship and trust. Ideal for bouquets and arrangements, it adds freshness, elegance, and a gentle aromatic touch to any floral display.",
    desc: "Freesias are fragrant blooms with trumpet-shaped petals that add freshness and a sweet aroma to bouquets. Their vivid colours and natural elegance make them perfect for lively and joyful arrangements.",
    maintain: [
      "Freesia is a delicate and graceful bloom admired for its soft petals and enchanting fragrance. Symbolizing friendship, trust, and innocence, this flower carries a gentle charm that feels fresh and uplifting. Its elegant form and sweet scent make freesia a beloved choice for refined bouquets and meaningful floral arrangements.",
    ],

    origin:
      "Available in a range of pastel shades and vibrant hues, freesia adds lightness, color, and a refreshing presence to floral designs. Its slender stems and clustered blossoms bring movement and natural beauty to bouquets, pairing beautifully with roses, lilies, and lisianthus. Ideal for weddings, celebrations, and heartfelt gifts, freesia brings warmth, joy, and a soft elegance to every arrangement.",

    symbolic: [
      "Freesia is cherished for its long-lasting freshness and unforgettable fragrance. Representing trust, friendship, and thoughtfulness, it is perfect for bouquets, centerpieces, and decorative floral displays. Its presence enhances special occasions and everyday spaces with freshness and charm. A flower that embodies purity, fragrance, and timeless beauty, freesia leaves a lasting and uplifting impression.",
    ],
    careContent:
      "To keep your Freesia flowers fresh and fragrant, provide them with plenty of water and change it every two days. Trim the stems at an angle to help them absorb water efficiently. Keep them in a cool area, away from direct sunlight, heat sources, and ripening fruits, to maintain their delicate, colorful, and aromatic blooms.",

    CareTips: [
      {
        Tip: "Trim stems at an angle every two days.",
      },
      {
        Tip: "Change water regularly to prevent bacterial growth.",
      },
      {
        Tip: "Keep away from direct sunlight and heat sources.",
      },
      {
        Tip: "Avoid placing near ripening fruits, which release ethylene gas.",
      },
    ],
    Culture:
      "Freesias are admired for their delicate, fragrant blooms and graceful charm. They symbolize innocence, trust, and thoughtfulness, making them perfect for bouquets, decorative arrangements, and meaningful gifts. Their elegant presence enhances weddings, celebrations, and everyday moments, remaining a flower that is gentle, meaningful, and unforgettable.",
  },
  {
    title: "Spray Rose 喷雾玫瑰 (Pēnwù méi guī)",
    img: "/images/blog-7.jpg",
    note: "Spray Roses are loved for their clustered blooms, delicate petals, and long-lasting freshness. Symbolizing joy and gratitude, they are ideal for bouquets and arrangements, adding fullness, elegance, and romantic charm to any floral display.",
    desc: "Tiny blooms that cluster bright, Soft as whispers, pure delight.Spray roses dance in gentle grace, Bringing charm to every space.",
    maintain: [
      "Spray Roses are charming and graceful blooms known for their clusters of small, delicate roses on a single stem. Symbolizing joy, gratitude, and gentle affection, they bring a soft romantic feel to any bouquet or floral arrangement. Their refined beauty and layered petals make spray roses a favorite for elegant and expressive floral designs.",
    ],

    origin:
      "Available in a range of pastel shades and vibrant hues, freesia adds lightness, color, and a refreshing presence to floral designs. Its slender stems and clustered blossoms bring movement and natural beauty to bouquets, pairing beautifully with roses, lilies, and lisianthus. Ideal for weddings, celebrations, and heartfelt gifts, freesia brings warmth, joy, and a soft elegance to every arrangement.",

    symbolic: [
      "Spray Roses are admired for their long-lasting freshness and versatile appeal. Representing happiness, appreciation, and heartfelt emotion, they are perfect for bouquets, centerpieces, and decorative arrangements alike. Their clustered blooms enhance events, celebrations, and everyday spaces with elegance and warmth. A flower that embodies charm, softness, and timeless romance, spray roses leave a lasting impression.",
    ],
    careContent:
      "To keep your spray roses fresh and vibrant, provide them with plenty of water and change it every two days. Trim the stems at an angle to improve water absorption. Keep them in a cool area, away from direct sunlight, heat sources, and ripening fruits, to preserve their delicate clusters of blooms.",

    CareTips: [
      {
        Tip: "Trim stems at an angle every two days.",
      },
      {
        Tip: "Change water regularly to prevent bacterial growth.",
      },
      {
        Tip: "Keep away from direct sunlight and heat sources.",
      },
      {
        Tip: "Avoid placing near ripening fruits, which release ethylene gas.",
      },
    ],
    Culture:
      "Spray Roses are admired for their delicate clusters of small blooms and graceful charm. They symbolize love, admiration, and thoughtfulness, making them perfect for bouquets, decorative arrangements, and meaningful gifts. Their elegant presence enhances weddings, celebrations, and everyday moments, remaining a flower that is tender, meaningful, and unforgettable.",
  },
  {
    title: "Foliage Leaves树叶 (Shù yè)",
    img: "/images/blog-2.jpeg",
    note: "Foliage leaves are essential for adding structure, balance, and freshness to floral arrangements. Ideal for bouquets and décor, they enhance the beauty of flowers while bringing a natural, elegant, and refreshing touch to every display.",
    desc: "Greens that whisper nature’s tone, Quiet beauty all their own. They frame the flowers, soft and still, Giving every bouquet shape and will.",
    maintain: [
      "Foliage leaves are the foundation of every beautiful floral arrangement, adding freshness, balance, and natural elegance. Their varied shapes, textures, and shades of green create depth and harmony, allowing blooms to stand out gracefully. Foliage brings a sense of nature and calm, making arrangements feel complete and visually pleasing.",
    ],

    origin:
      "From soft, delicate leaves to bold and structured greenery, foliage complements all types of flowers and bouquet styles. It enhances color contrast, adds movement, and creates a refined, organic look in both modern and classic designs. Ideal for bouquets, centerpieces, and decorative arrangements, foliage leaves provide a timeless and versatile backdrop for floral artistry.",

    symbolic: [
      "Foliage leaves are admired for their durability and versatile appeal. Symbolizing growth, freshness, and renewal, they add life and balance to floral designs. Perfect for weddings, events, and everyday arrangements, foliage elevates spaces with natural beauty and elegance. A timeless element in floristry, foliage leaves enhance every arrangement with texture, harmony, and lasting charm.",
    ],
    careContent:
      "To keep your foliage leaves fresh and vibrant, provide them with plenty of water and change it every twodays. Trim the stems at an angle to help them absorb water efficiently. Keep them in a cool area, away from direct sunlight, heat sources, and ripening fruits to maintain their lush green appearance.",

    CareTips: [
      {
        Tip: "Trim stems at an angle every two days.",
      },
      {
        Tip: "Change water regularly to prevent bacterial growth.",
      },
      {
        Tip: "Keep away from direct sunlight and heat sources.",
      },
      {
        Tip: "Avoid placing near ripening fruits, which release ethylene gas.",
      },
    ],
    Culture:
      "Foliage leaves are admired for their lush greenery and natural elegance. They symbolize growth, harmony, and renewal, making them perfect for bouquets, floral arrangements, and decorative accents. Their vibrant presence enhances weddings, celebrations, and everyday moments, remaining an element that is graceful, meaningful, and unforgettable.",
  },
  {
    title: "Dusty Miller 银叶菊 (Yín yè jú)",
    img: "/images/blog-3.jpeg",
    note: "Dusty Miller is cherished for its soft silver leaves, velvety texture, and elegant contrast. Ideal as a foliage accent in bouquets and arrangements, it adds balance, sophistication, and a modern, refined touch to floral designs.",
    desc: "Silver leaves like winter’s glow, Soft and velvety as falling snow. Dusty Miller stands serene, A timeless touch of silver-green.",
    maintain: [
      "Dusty Miller is a distinctive foliage plant admired for its soft, silvery-gray leaves and velvety texture. Its unique color adds contrast and elegance, creating a refined backdrop that highlights surrounding blooms beautifully. Known for its calm and sophisticated presence, Dusty Miller brings a modern, graceful touch to floral arrangements.",
    ],

    origin:
      "With its finely textured, sculpted leaves, Dusty Miller adds depth, balance, and visual interest to bouquets and decorative designs. It pairs effortlessly with roses, peonies, lisianthus, and seasonal flowers, enhancing both pastel and bold color palettes. Ideal for weddings, events, and elegant décor, Dusty Miller offers a timeless, airy charm that elevates any floral style.",

    symbolic: [
      "Dusty Miller is valued for its durability and long-lasting freshness. Symbolizing calmness, protection, and subtle beauty, it is perfect for bouquets, centerpieces, and decorative arrangements. Its silvery foliage enhances celebrations, special occasions, and everyday spaces with a soft, luxurious feel. A classic foliage choice, Dusty Miller adds elegance, texture, and lasting appeal to every floral creation.",
    ],
    careContent:
      "To keep your Dusty Miller fresh and silvery, provide it with plenty of water and change it every two days. Trim the stems at an angle to help them absorb water efficiently. Keep Dusty Miller in a cool area, away from direct sunlight, heat sources, and ripening fruits to maintain its soft, silvery foliage.",

    CareTips: [
      {
        Tip: "Trim stems at an angle every two days.",
      },
      {
        Tip: "Change water regularly to prevent bacterial growth.",
      },
      {
        Tip: "Keep away from direct sunlight and heat sources.",
      },
      {
        Tip: "Avoid placing near ripening fruits, which release ethylene gas.",
      },
    ],
    Culture:
      "Dusty Miller is admired for its soft, silvery foliage and subtle elegance. It symbolizes resilience, sophistication, and understated beauty, making it perfect for bouquets, decorative arrangements, and complementary accents. Its refined presence enhances weddings, celebrations, and everyday moments, remaining an element that is graceful, meaningful, and unforgettable.",
  },
  {
    title: "Eucalyptus 桉树 (Ān shù)",
    img: "/images/blog-4.jpeg",
    note: "Eucalyptus is loved for its refreshing aroma, soft foliage, and modern elegance. Used as a foliage accent, it adds texture, balance, and a calming, sophisticated touch to floral designs and decorative arrangements.",
    desc: "Fragrant leaves with calming air, A touch of blue-green everywhere.Eucalyptus whispers fresh and free, A soothing gift from nature’s tree.",
    maintain: [
      "Eucalyptus is a graceful foliage admired for its soft, rounded leaves and refreshing natural fragrance. Its muted green and silvery tones create a calming, modern aesthetic that blends effortlessly with a wide range of floral styles. Known for its clean and elegant presence, eucalyptus adds freshness and sophistication to any arrangement.",
    ],

    origin:
      "With its flowing stems and delicate leaf structure, eucalyptus brings movement, texture, and balance to bouquets and decorative designs. It pairs beautifully with roses, peonies, lisianthus, and seasonal blooms, enhancing both romantic and contemporary arrangements. Ideal for weddings, events, and premium décor, eucalyptus adds a light, airy charm that elevates floral compositions.",

    symbolic: [
      "Eucalyptus is valued for its long-lasting freshness and versatile appeal. Symbolizing healing, protection, and renewal, it is perfect for bouquets, centerpieces, and ornamental arrangements alike. Its presence enhances celebrations, special occasions, and everyday spaces with a soothing, refined elegance. A timeless foliage choice, eucalyptus leaves a lasting impression with its natural beauty and calming essence.",
    ],
    careContent:
      "To keep your eucalyptus fresh and aromatic, provide it with plenty of water and change it every two days. Trim the stems at an angle to help them absorb water efficiently. Keep eucalyptus in a cool area, away from direct sunlight, heat sources, and ripening fruits to maintain its fragrant, silvery-green foliage.",

    CareTips: [
      {
        Tip: "Trim stems at an angle every two days.",
      },
      {
        Tip: "Change water regularly to prevent bacterial growth.",
      },
      {
        Tip: "Keep away from direct sunlight and heat sources.",
      },
      {
        Tip: "Avoid placing near ripening fruits, which release ethylene gas.",
      },
    ],
    Culture:
      "Eucalyptus is admired for its aromatic, silvery-green leaves and calming presence. It symbolizes protection, healing, and renewal, making it perfect for bouquets, decorative arrangements, and thoughtful accents. Its serene presence enhances weddings, celebrations, and everyday moments, remaining an element that is soothing, meaningful, and unforgettable.",
  },
  {
    title: "Green Ball (绿绒球 Lǜ róng qiú)",
    img: "/images/greenball.jpg",
    note: "Green Ball is loved for its fuzzy texture, vibrant green color, and modern appeal. It adds volume, balance, and a refreshing natural touch to bouquets and decorative floral arrangements.",
    desc: "Green Ball is a unique, fuzzy, moss-like bloom that adds bold texture and a modern touch to bouquets. Its vibrant green color brings freshness and volume, making any arrangement look stylish and lively.",
    maintain: [
      "Green Ball is a distinctive bloom admired for its soft, moss-like texture and perfectly rounded form. Its vibrant green hue brings a refreshing, modern touch to floral arrangements, creating an eye-catching yet calming visual effect. This unique flower adds depth and character, making designs feel fresh, stylish, and contemporary.",
    ],

    origin:
      "With its plush appearance and sculptural shape, Green Ball enhances bouquets by adding volume, texture, and contrast. It pairs beautifully with roses, lilies, orchids, and seasonal flowers, balancing bright colors with its soothing green tone. Ideal for modern bouquets, event décor, and artistic floral designs, Green Ball introduces a playful yet refined charm.",

    symbolic: [
      "Green Ball is valued for its durability and long-lasting freshness, making it a reliable choice for both decorative and celebratory arrangements. Symbolizing renewal, harmony, and positive energy, it fits perfectly into bouquets, centerpieces, and contemporary installations. A versatile accent flower, Green Ball elevates every arrangement with its unique texture and refreshing elegance.",
    ],
    careContent:
      "To keep your Green Ball flowers fresh and vibrant, provide them with plenty of water and change it every two days. Trim the stems at an angle to help them absorb water efficiently. Keep them in a cool area, away from direct sunlight, heat sources, and ripening fruits to maintain their round, lush appearance.",

    CareTips: [
      {
        Tip: "Trim stems at an angle every two days.",
      },
      {
        Tip: "Change water regularly to prevent bacterial growth.",
      },
      {
        Tip: "Keep away from direct sunlight and heat sources.",
      },
      {
        Tip: "Avoid placing near ripening fruits, which release ethylene gas.",
      },
    ],
    Culture:
      "Green Ball is admired for its vibrant, spherical blooms and unique texture. It symbolizes harmony, vitality, and abundance, making it perfect for bouquets, decorative arrangements, and thoughtful accents. Its striking presence enhances weddings, celebrations, and everyday moments, remaining an element that is lively, meaningful, and unforgettable.",
  },
  {
    title: " Panicum (柳枝草 Liǔ zhī cǎo)",
    img: "/images/blog-32.jpg",
    note: "Panicum is a lightweight, flowing grass that adds texture, movement, and natural elegance to bouquets and floral arrangements.",
    desc: "Panicum features airy, delicate seed heads that gently sway and add movement to bouquets. It brings a soft, garden-like, natural feel and perfectly complements bright and pastel arrangements.",
    maintain: [
      "Panicum is a graceful ornamental grass admired for its airy texture and naturally flowing form. Its delicate stems and soft seed heads bring movement and lightness to floral arrangements, creating a relaxed yet elegant appearance. This versatile filler adds a subtle, organic charm that enhances both modern and rustic designs.",
    ],

    origin:
      "Known for its neutral tones and feathery structure, Panicum blends seamlessly with fresh and dried flowers alike. It adds depth, softness, and balance without overpowering focal blooms. Ideal for bouquets, wedding décor, and contemporary floral styling, Panicum introduces a calm, natural rhythm to every arrangement.",

    symbolic: [
      "Panicum is valued for its durability and long-lasting beauty, making it a favorite for decorative arrangements and installations. Symbolizing harmony, simplicity, and natural elegance, it complements a wide range of floral compositions. A timeless filler, Panicum elevates designs with effortless grace and refined texture.",
    ],
    careContent:
      "To keep your Panicum fresh and airy, provide it with plenty of water and change it every two days. Trim the stems at an angle to help them absorb water efficiently. Keep Panicum in a cool area, away from direct sunlight, heat sources, and ripening fruits to maintain its delicate, feathery appearance.",

    CareTips: [
      {
        Tip: "Trim stems at an angle every two days.",
      },
      {
        Tip: "Change water regularly to prevent bacterial growth.",
      },
      {
        Tip: "Keep away from direct sunlight and heat sources.",
      },
      {
        Tip: "Avoid placing near ripening fruits, which release ethylene gas.",
      },
    ],
    Culture:
      "Panicum is admired for its delicate, wispy stems and graceful movement. It symbolizes flexibility, elegance, and tranquility, making it perfect for bouquets, decorative arrangements, and subtle accents. Its gentle presence enhances weddings, celebrations, and everyday moments, remaining an element that is refined, meaningful, and unforgettable.",
  },
  {
    title: "Ammi Majus(白花蛇舌草 Báihuāshéshé cǎo)",
    img: "/images/blog-34.jpg",
    note: "Ammi Majus is treasured for its delicate, lacy clusters, long-lasting freshness, and elegant presence. Perfect as a filler or accent in bouquets and arrangements, it adds texture, refinement, and subtle charm to any floral display.",
    desc: "Ammi Majus, often called Queen Anne’s Lace, has delicate lace-like white clusters that bring softness and a natural garden feel to any bouquet. It is widely used in romantic and airy floral designs.",
    maintain: [
      "Ammi Majus is a delicate, lacy white bloom admired for its airy, intricate structure and subtle elegance. Often used as a filler flower in bouquets and arrangements, it adds softness, texture, and a touch of refinement. Its delicate clusters exude grace and charm, making it an ideal companion to more vibrant blooms or as a standalone accent. Ammi Majus is cherished for its understated beauty and timeless appeal.",
    ],

    origin:
      "With its fine, feathery appearance, Ammi Majus creates depth and dimension in floral designs. It complements a wide variety of flowers, from roses and peonies to lilies and tulips, enhancing every bouquet with texture and elegance. Perfect for weddings, anniversaries, or refined decorative arrangements, Ammi Majus brings a sense of sophistication, lightness, and airy charm to any setting.",

    symbolic: [
      "Ammi Majus is valued for its long-lasting freshness and versatile use in floral arrangements. Symbolizing protection, elegance, and delicacy, it adds subtle beauty and balance to every bouquet. Ideal for centerpieces, bouquets, or decorative accents, Ammi Majus enhances celebrations, events, and everyday spaces with its refined charm. A flower that embodies grace, delicacy, and timeless sophistication, it leaves a gentle yet memorable impression.",
    ],
    careContent:
      "To keep your Ammi Majus flowers fresh and delicate, provide them with plenty of water and change it every two days. Trim the stems at an angle to improve water absorption. Place them in a cool area, away from direct sunlight, heat sources, and ripening fruits, to maintain their airy, lacy blooms.",

    CareTips: [
      {
        Tip: "Trim stems at an angle every two days.",
      },
      {
        Tip: "Change water regularly to prevent bacterial growth.",
      },
      {
        Tip: "Keep away from direct sunlight and heat sources.",
      },
      {
        Tip: "Avoid placing near ripening fruits, which release ethylene gas.",
      },
    ],
    Culture:
      "Ammi Majus is admired for its delicate, lacy white blooms and subtle elegance. It symbolizes protection, clarity, and gentle beauty, making it ideal for bouquets, decorative arrangements, and thoughtful gifts. Its refined presence enhances weddings, celebrations, and everyday moments, remaining a flower that is graceful, meaningful, and unforgettable.",
  },
  {
    title: "Eustoma / Lisianthus (桔梗花 Júgěng huā)",
    img: "/images/blog-30.jpg",
    note: "Eustoma (Lisianthus) is loved for its rose-like petals, soft colors, and long-lasting freshness. Symbolizing appreciation and heartfelt emotion, it is ideal for bouquets, arrangements, and special occasions, adding elegance, softness, and emotional depth to any floral display.",
    desc: "Eustoma, also called Lisianthus, features rose-like petals with a soft, romantic appeal. Its silky texture and pastel tones bring elegance, charm, and a premium look to any arrangement.",
    maintain: [
      "Eustoma, also known as Lisianthus, is a graceful bloom admired for its soft, rose-like petals and refined elegance. Symbolizing appreciation, gratitude, and heartfelt emotion, this flower carries a gentle charm that feels both romantic and sophisticated. Its delicate appearance and smooth texture make it a favorite choice for premium bouquets and elegant floral arrangements.",
    ],

    origin:
      "Available in soothing pastels and rich, elegant hues, Eustoma blends beautifully with both classic and modern floral designs. Its layered petals add softness, volume, and balance to bouquets, complementing flowers such as roses, peonies, and lilies. Ideal for weddings, anniversaries, or meaningful gifts, Lisianthus brings calm beauty and emotional depth to every arrangement.",

    symbolic: [
      "Eustoma is cherished for its long-lasting freshness and versatile nature. Representing appreciation, sincerity, and graceful love, it is perfect for bouquets, centerpieces, and decorative floral displays. Its serene presence enhances celebrations, special events, and everyday spaces with elegance and warmth. A flower that embodies refinement, emotion, and timeless beauty, Eustoma leaves a lasting and memorable impression.",
    ],
    careContent:
      "To keep your Eustoma / Lisianthus flowers fresh and elegant, provide them with plenty of water and change it every two days. Trim the stems at an angle to improve water absorption. Keep them in a cool area, away from direct sunlight, heat sources, and ripening fruits, to maintain their delicate, rose-like blooms.",

    CareTips: [
      {
        Tip: "Trim stems at an angle every two days.",
      },
      {
        Tip: "Change water regularly to prevent bacterial growth.",
      },
      {
        Tip: "Keep away from direct sunlight and heat sources.",
      },
      {
        Tip: "Avoid placing near ripening fruits, which release ethylene gas.",
      },
    ],
    Culture:
      "Eustoma, also known as Lisianthus, is admired for its delicate, rose-like blooms and graceful elegance. It symbolizes appreciation, charisma, and heartfelt emotions, making it perfect for bouquets, decorative arrangements, and thoughtful gifts. Its refined presence enhances weddings, celebrations, and everyday moments, remaining a flower that is elegant, meaningful, and unforgettable.",
  },
  {
    title: "Matthiola / Stock Flower (紫罗兰 Zǐluólán)",
    img: "/images/blog-8.jpg",
    note: "Matthiola (Stock Flower) is loved for its rich fragrance, clustered blooms, and symbolic meanings of affection and lasting beauty. Ideal for bouquets and arrangements, it adds fullness, elegance, and a romantic aroma to any floral display.",
    desc: "Matthiola, commonly known as Stock, is loved for its strong fragrance and dense clusters of ruffled blooms. It gives bouquets a luxurious, full appearance with a fresh, romantic charm.",
    maintain: [
      "Matthiola, commonly known as Stock Flower, is a classic and fragrant bloom admired for its dense clusters and soft, romantic appeal. Symbolizing lasting beauty, affection, and contentment, this flower carries a timeless charm that feels warm and elegant. Its velvety petals and gentle fragrance make it a favorite for refined bouquets and traditional floral arrangements.",
    ],

    origin:
      "Available in soothing pastels and rich, elegant hues, Eustoma blends beautifully with both classic and modern floral designs. Its layered petals add softness, volume, and balance to bouquets, complementing flowers such as roses, peonies, and lilies. Ideal for weddings, anniversaries, or meaningful gifts, Lisianthus brings calm beauty and emotional depth to every arrangement.",

    symbolic: [
      "Matthiola is cherished for its long-lasting freshness and pleasing fragrance. Representing loyalty, happiness, and enduring bonds, it is perfect for bouquets, centerpieces, and decorative floral displays. Its presence enhances events and everyday spaces with warmth and elegance. A flower that embodies romance, fragrance, and timeless beauty, stock flowers leave a soft yet unforgettable impression.",
    ],
    careContent:
      "To keep your Matthiola / Stock flowers fresh and fragrant, provide them with plenty of water and change it every two days. Trim the stems at an angle to help them absorb water efficiently. Place them in a cool area, away from direct sunlight, heat sources, and ripening fruits, to preserve their vibrant colors and sweet scent.",

    CareTips: [
      {
        Tip: "Trim stems at an angle every two days.",
      },
      {
        Tip: "Change water regularly to prevent bacterial growth.",
      },
      {
        Tip: "Keep away from direct sunlight and heat sources.",
      },
      {
        Tip: "Avoid placing near ripening fruits, which release ethylene gas.",
      },
    ],
    Culture:
      "Matthiola, also known as Stock Flower, is admired for its fragrant, clustered blooms and elegant appearance. It symbolizes lasting beauty, affection, and a sense of commitment, making it perfect for bouquets, decorative arrangements, and meaningful gifts. Its graceful presence enhances weddings, celebrations, and everyday moments, remaining a flower that is charming, meaningful, and unforgettable.",
  },
  {
    title: "FILLER FLOWERS 补花 (Bǔ huā)",
    img: "/images/blog-18.jpg",
    desc: "Filler flowers are delicate, small-bloom varieties used to add volume, softness, texture,and balance to floral arrangements. They enhance the beauty of main flowers, bring visual depth, and create a natural, airy, and elegant look in bouquets.",
    note: "Filler flowers add texture, balance, and fullness, beautifully completing bouquets and floral arrangements with natural elegance.",
    maintain: [
      "Filler flowers play a vital role in creating balanced and visually complete floral arrangements. They add softness, depth, and texture, gracefully connecting focal blooms and enhancing the overall harmony of a bouquet. Subtle yet essential, filler flowers bring life and flow to floral designs without overpowering the main flowers.",
    ],

    origin:
      "Known for their delicate forms and versatile nature, filler flowers complement a wide variety of blooms and color palettes. They introduce movement, volume, and natural elegance, making arrangements feel fuller and more refined. Perfect for bouquets, centerpieces, and decorative installations, filler flowers elevate every design with gentle beauty.",

    symbolic: [
      "Filler flowers symbolize support, unity, and completeness, representing the details that make moments truly special. Loved for their adaptability and lasting freshness, they are essential in both classic and modern floral styling. With their understated charm, filler flowers create a polished, harmonious finish that enhances every floral creation.",
    ],
    careContent:
      "To keep your filler flowers fresh and vibrant, provide them with plenty of water and change it every two days. Trim the stems at an angle to help them absorb water efficiently. Keep them in a cool area, away from direct sunlight, heat sources, and ripening fruits to maintain their delicate, supportive blooms in arrangements.",

    CareTips: [
      {
        Tip: "Trim stems at an angle every two days.",
      },
      {
        Tip: "Change water regularly to prevent bacterial growth.",
      },
      {
        Tip: "Keep away from direct sunlight and heat sources.",
      },
      {
        Tip: "Avoid placing near ripening fruits, which release ethylene gas.",
      },
    ],
    Culture: "Filler flowers are admired for their delicate forms and subtle charm. They symbolize support, harmony, and completeness, making them perfect for bouquets, decorative arrangements, and accentuating primary blooms. Their understated presence enhances weddings, celebrations, and everyday moments, remaining an element that is graceful, meaningful, and unforgettable.",
  },
  {
    title: "Jasmine ( 茉莉花 Mòlì huā)",
    img: "/images/jasmine.jpg",
    desc: "Jasmine flowers are small, delicate, and highly fragrant blooms commonly used in bouquets, garlands, hair adornments, and decorative floral arrangements. They enhance floral designs by adding softness, natural beauty, and a refreshing aroma. Jasmine also brings a serene elegance while carrying deep cultural and emotional significance.",
    note: "Jasmine symbolizes purity, love, grace, and serenity, representing heartfelt emotions and timeless beauty. Loved for their soothing fragrance and adaptability, jasmine flowers are essential in both classic and modern floral styling. With their understated charm and aromatic presence, jasmine flowers create a polished, graceful finish that enhances every floral creation with natural elegance.",
    maintain: [
      "Jasmine flowers are cherished for their enchanting fragrance and timeless elegance, bringing purity, warmth, and emotional depth to floral arrangements. With their soft white blooms and captivating scent, jasmine flowers gently enhance bouquets, garlands, and decorative installations, creating a sense of calm and romance. Delicate yet expressive, jasmine adds a graceful flow that elevates every floral design without overwhelming the composition.",
    ],

    origin:
      "Known for their soothing aroma and symbolic beauty, jasmine flowers blend effortlessly with both traditional and contemporary styles. They introduce natural freshness, subtle texture, and cultural richness, making arrangements feel more intimate and meaningful. Perfect for weddings, ceremonies, festive décor, and daily floral styling, jasmine flowers leave a lasting impression through their fragrance and simplicity.",

    symbolic: [
      "Jasmine flowers symbolize purity, love, peace, and positive energy—representing emotions that linger long after the moment. Valued for their freshness and versatility, jasmine flowers bring a refined, harmonious finish that enhances every floral creation with gentle elegance.",
    ],
    careContent: "To keep your jasmine flowers fresh and fragrant, provide them with plenty of water and change it every two days. Trim the stems at an angle to help them absorb water efficiently. Keep them in a cool area, away from direct sunlight, heat sources, and ripening fruits to preserve their delicate blooms and sweet aroma.",

    CareTips: [
      {
        Tip: "Trim stems at an angle every two days.",
      },
      {
        Tip: "Change water regularly to prevent bacterial growth.",
      },
      {
        Tip: "Keep away from direct sunlight and heat sources.",
      },
      {
        Tip: "Avoid placing near ripening fruits, which release ethylene gas.",
      },
    ],
    Culture: "Jasmine is admired for its delicate, fragrant blossoms and elegant charm. It symbolizes purity, love, and spiritual devotion, making it perfect for bouquets, decorative arrangements, and ceremonial garlands. Its enchanting presence enhances weddings, celebrations, and everyday moments, remaining a flower that is graceful, meaningful, and unforgettable.",
  },
];
